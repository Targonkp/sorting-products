...
document.getElementById('filters-brand').addEventListener('click', (event) => {
...
document.getElementById('filters-price').addEventListener('click', (event) => {
  if (event.target.tagName === 'LABEL') {
    let [min, max] = event.target.textContent.match(/\d+/g);
    newElementsList = tvlist.filter(
...
document
  .getElementById('filters-diagonal')
  .addEventListener('click', (event) => {
    if (event.target.tagName === 'LABEL') {
      let filterDiagonal = parseFloat(event.target.textContent);
      newElementsList = tvlist.filter(
        (item) => parseFloat(item.diagonal) === filterDiagonal
      );
      createPaginationEl(newElementsList); //пагинация
      createProducts(1, newElementsList); //отображение куска массива
    }
...
let chekcbox = document
  .getElementById('filters-price')
...
function filter() {
  ...
  [...chekcbox].forEach((elem) => {
    if (elem.checked) {
      newArray.push(
        elem.closest('.checkbox').querySelector('label').textContent
      );
    }
  });
...
document.getElementById('filters-price').addEventListener('change', filter);
